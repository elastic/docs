...
=== Alternate languages for examples

The build supports finding "alternate languages" for examples that allows users
to specify their preferred client or language. You can do this by passing
`--examples` to the build like this:

[source,bash]
----------------------------
cd docs/
./build_docs --doc README.asciidoc --open --asciidoctor \
    --examples js:integtest/readme_examples/js \
    --examples csharp:integtest/readme_examples/csharp
----------------------------
