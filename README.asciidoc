...
[[glossary-pop-ups]]
== Inline pop-ups for glossary terms

Rather than linking to glossary pages, you can use an inline definition pop-up.
...
Definition pop-ups can be assigned to attributes, consume attributes, and contain links.
To use special characters in definition pop-ups, including commas, wrap the definition in double quotation marks.

[source,asciidoc]
----
// Assign the definition to an attribute
:run-short-def: To move at a speed faster than a walk. See <<run-link,run>> to learn more.

// Create a definition pop-up and assign it to an attribute
:run-def: definition:[run,"{run-short-def}"]

// Reference the definition attribute to create a pop-up in the docs
run::
{run-short-def}. I like to {run-def}.
----
