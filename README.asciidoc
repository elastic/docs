...
The `m1-workaround` branch allows building and viewing the docs on machines with
Apple M1 processors. The only difference between this branch and the master
branch is that the `--open` flag to `./build_docs` won't work, and instead
you'll need to run your build command (either `./build_docs` or one of the
various aliases) first, and then run `docserve` (a new alias that exists only in
...
This branch will be kept up to date with the `master`
branch as much as possible, but can be updated at any point with:
