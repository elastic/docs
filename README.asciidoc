...
In general, you should avoid creating books that have parts.
Parts add complexity to the book structure and make it harder to
...
Links are a powerful way to direct users to relevant
resources, but keep in mind that excessive linking can
distract users and redirect them away from the topic's
main point.
...
----------------------------------
[[para-id]]
Use the ID `para-id` to link to this paragraph.
----------------------------------

You can use _attributes_ within IDs to set IDs dynamically:  

[source,asciidoc]
----------------------------------
[[para-id-{myattribute}]]
When myattribute is set to somevalue, use the ID
`para-id-somevalue` to link to this paragraph.
----------------------------------
...
[NOTE]
===============================
Use the ID `note-id` to link to this note.
===============================
----------------------------------
...
It's generally best to avoid using alternative link text
because the link text may get stale over time. If you do
use link text, be specific. Avoid using vague text, such as
“click here”. 
...
You can also use this approach when you have content
that you want to include in multiple topics or books.
Instead of duplicating the content, you can create a
single Asciidoc file and include it wherever it's
required. Just keep in mind that IDs must be unique
within a book, so you can't use IDs in a shared
file if the file will be included multiple times
within the same book. To work around this, you can use
an attribute in the ID and set the attribute to a unique
value wherever the shared file is included.
...
If you decide to use tables, limit the number of columns
to 2 or 3, and make sure the text doesn't overrun the right margin.
